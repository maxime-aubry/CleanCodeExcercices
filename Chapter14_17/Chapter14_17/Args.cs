using Chapter14_17.Marshalers;
using System;
using System.Collections.Generic;

namespace Chapter14_17
{
    public class Args
    {
        ...
        private void parseArguments()
        {
            for (this.currentArgument = this.argsList.GetEnumerator(); this.currentArgument.MoveNext();)
            {
                ...
                if (argument.StartsWith('-'))
                    parseArgumentValues(argument.Substring(1));
            }
        }

        private void parseArgumentValues(string parameterNames)
        {
            for (int i = 1; i < parameterNames.Length; i++)
                parseArgumentValue(parameterNames[i]);
        }

        private void parseArgumentValue(char parameterName)
        {
            ArgumentMarshaler am = this.marshalers.GetValueOrDefault(parameterName);
            if (am == null)
                throw new ArgsException(ArgsException.ErrorCode.UNEXPECTED_ARGUMENT, parameterName, null);

            this.argsFound.Add(parameterName);
            ...
            {
                e.setErrorArgumentId(parameterName);
                throw e;
            }
        }
